using BenchmarkTools, Dierckx, NLsolve, Roots
...
begin
    ...
    function EkN(kx::AbstractArray{T}, ky::AbstractArray{T}) where T <: Float64
        I1  = (1 - p) * I
    ...
    function EkN_(kx::AbstractArray{T}, ky::AbstractArray{T}) where T <: Float64
        I1 = (1 - p) * I
        csx = cos.(-kx .- 2 * α_x); csy = cos.(-ky)
        snx = sin.(-kx / 2 .- α_x); sny = sin.(-ky / 2)
        ...
        Res_eq_3[:, :, 1] = D11_ +
                          + D22_ +
                          + D33_ ./ Kk_
        Res_eq_3[:, :, 2] = D11_ .* D22_ +
                          - D12_ .* D12_ +
                          + D11_ .* D33_ ./ Kk_ +
                          - D23_ .* D23_ ./ Kk_ +
                          - D13_ .* D13_ ./ Kk_ +
                          + D22_ .* D33_ ./ Kk_
        Res_eq_3[:, :, 3] = D11_ .* D22_ .* D33_ ./ Kk_ +
                          - D11_ .* D23_ .* D23_ ./ Kk_ +
                          + D12_ .* D13_ .* D23_ ./ Kk_ +
                          + D12_ .* D13_ .* D23_ ./ Kk_ +
                          - D12_ .* D12_ .* D33_ ./ Kk_ +
    ...
    function npSC(X)
        ...
        """ ====== Finding Deltas: ========================================= """
        nlsolve(EquationsSystemForDeltas, [.01;.01;.01;.01;.01;.01])
        """ ================================================================ """
        d_sym = (cos.(kx) - cos.(ky))
        D14k = d_sym * Dlt1
        D25k = d_sym * Dlt2
        ...
        ESC = zeros(N, N, 6)
        E = EkN(kx, ky) .- mu; E1 = E[:, :, 1]; E2 = E[:, :, 2]; E3 = E[:, :, 3]
        E_=EkN_(kx, ky) .+ mu; E1_=E_[:, :, 3]; E2_=E_[:, :, 2]; E3_=E_[:, :, 1]
        Ek = sqrt.(((E1 - E1_) / 2) .^ 2 + D14k .^ 2 + D25k .^ 2 +
                                         + D36k .^ 2 ./ Kk ./ Kk_)
    ...
    function muSC(vmu::Float64)
        global x, mu
        mu = vmu
         return npSC(1) - x
    end

    function EquationsSystemForDeltas(Dlt::AbstractArray{T}) where T <: Float64
        global kx, ky, T, mu, Dlt1, Dlt2, Dlt3, Dlt4, Dlt5, Dlt6
        global ε_p, τ_, t, α_x, x, p, C1, C2, C3, J, I, Up, V2
        ...
        ESC[:, :, 4] = (E1 + E1_) / 2 - Ek; ESC[:, :, 5] = E2_; ESC[:, :, 6] = E3_
        """================================================================= """
        Fx1 = Fx2 = Fx3 = Fx4 = Fx5 = Fx6 = zeros(N, N)
    ...
    function curSC(float)
        global kx, ky, T, mu, Dlt1, Dlt2, Dlt3, Dlt4, Dlt5, Dlt6
        global ε_p, τ_, t, α_x, x, p, C1, C2, C3, J, I, Up, V2

        mu = fzero(muSC, [-10, -0], xatol=1e-10)
        """ ====================================================================="""
        d_sym = (cos.(kx) - cos.(ky))
        D14k = d_sym * Dlt1
        D25k = d_sym * Dlt2
        ...
        ESC = zeros(N, N, 6)
        E = EkN(kx, ky) .- mu; E1 = E[:, :, 1]; E2 = E[:, :, 2]; E3 = E[:, :, 3]
        E_=EkN_(kx, ky) .+ mu; E4= E_[:, :, 3]; E5 =E_[:, :, 2]; E6 =E_[:, :, 1]
        Ek = sqrt.(((E1 - E4) / 2) .^ 2 + D14k .^ 2 + D25k .^ 2 +
                                        + D36k .^ 2 ./ Kk ./ Kk_)
